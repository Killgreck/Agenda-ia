...
Tu respuesta:`;
  ...
  // Mantener la ruta pública para pruebas temporales
  app.post("/api/public-chat", async (req: Request, res: Response) => {
    try {
      ...
      log(`Recibido mensaje para chat público (temporal): "${message}"`, 'gemini');
      
      const prompt = `Eres un asistente inteligente para una aplicación de calendario y productividad.
...
Tu respuesta:`;
  ...
  log('Rutas de chat registradas en /api/chat (privado) y /api/public-chat (público)', 'gemini');
}